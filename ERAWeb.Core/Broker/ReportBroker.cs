...
namespace ERAWeb.Core.Broker
{
    public class ReportBroker : BrokerBase, IReportBroker
    {
        ...
        public async Task<List<UserModel>> GetUserReport(ReportSearchModel model)
        {
            ...
            response = await Task.Run(() => proxy.GetUserReport(new ReportRequest() { CompanyName = model.CompanyName, Email = model.Email, LastName = model.LastName, TestIdentifier = model.TestIdentifier }));
